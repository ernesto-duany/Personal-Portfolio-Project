(function () {
	...
	async function populateAboutMe() {
		try {
			...
			headshotImg.src = "/starter/images/pfp.jpeg";
	...
	function updateChars() {
		...
		if (used > MAX_MESSAGE) {
			charsLeftEl.classList.add('error');
		} else {
			charsLeftEl.classList.remove('error');
		}
	}
	...
	function validateMessageRealTime() {
		...
		if (ILLEGAL_RE.test(msgVal)) {
			...
			messageError.textContent = `Message must be ${MAX_MESSAGE} characters or less.`;
		} else {
	...
	function handleFormSubmit() {
		...
		if (!msgVal) {
			...
			if (messageError) messageError.textContent = `Message must be ${MAX_MESSAGE} characters or less.`;
			isValid = false;
		}
